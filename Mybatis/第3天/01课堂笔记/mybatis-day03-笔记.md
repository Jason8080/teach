#### 01复习【了解】

##### 目标

- 回顾Mybatis的常用功能为今天的高级功能打下基础

##### 步骤

###### 面试题

- \#{}与${}的区别是什么 ?

  - **\#{}**是Mybatis框架在映射文件中取值的 **占位符**
  - **${}**是Mybatis框架在映射文件中取值的 **拼接符**

  

  - **\#{}**获取的值参与预编译效验特殊字符具**有防SQL注入的作用**
  - **${}**获取的值直接拼接在执行语句中**有SQL注入的风险**

  

  - **\#{}**可以填写 **#{*}** 任何名称获取基本数据类型的参数
  - **\${}**只能填写 **${value}** 来获取基本数据类型的参数

- Mybatis映射文件中如何返回自增长主键值 ?

  - **不支持自增长的数据库** 可以使用 **SelectKey** 标签在 **Insert语句执行后** 执行查询最后主键值的语句并反射到参数对象中的某一个属性

    ```xml
    <selectKey resultType="int" keyColumn="id" keyProperty="id" order="AFTER">
        -- 查询当前连接中最后一个自增长主键值
        select LAST_INSERT_ID()
    </selectKey>
    ```

  - **支持自增长的数据库**  可以在Insert标签中添加**useGeneratedKeys**和**keyProperty**属性达到效果并且本质与<selectKey标签相同

###### 开发形式

- 代理开发
- 传统开发



##### 操作

###### 创建新模块

- 接下来学习的是连接池相关内容
- 模块名称: mybatis-day03-pool


###### 环境测试

```java

```


##### 小结

- 传统开发和代理开发的区别是什么?
  - 



#### 02连接池类型配置【了解】

##### 目标

- 配置不同的连接池类型

##### 步骤

- 配置dataSource type=POOLED
- 配置dataSource type=UNPOOLED
- 配置dataSource type=**jndi实现类**

##### 操作

###### POOLED: 内部连接池

- 配置

  ```xml
  
  ```

- 测试

  ```java
  
  ```

###### UNPOOLED: 不用连接池

- 配置

  ```xml
  
  ```

- 测试

  ```java
  
  ```

###### jndi: 外部连接池 

- 创建jndi实现类

  ```java
  
  ```

- 配置

  ```xml
  
  ```

- 测试

  ```java
  
  ```

##### 小结

- POOLED采用了框架提供的连接池技术, 连接关闭只是放回连接池
- UNPOOLED不采用连接池, 每次执行都是一个新连接
- JNDI采用外部的连接, 同样达到连接关闭只是放回连接池的效果



#### 03动态语句标签 - if【掌握】

##### 目标

- ==根据 **名称和性别** 查找用户==
- ==只根据 **名称** 查找用户==

##### 步骤

1. 创建新项目, 引入依赖
2. UserMapper.java: 增加 **findBy** 操作方法
3. userMapper.xml: 增加 **findBy** 操作配置
4. **添加测试**

##### 操作

- ==根据 **名称和性别** 查找用户==

###### 测试

```java

```

- ==只根据 **名称** 查找用户==

1. 直接注释其他条件欲达到效果



##### 小结

- if标签的作用是什么?
  - 
- if条件中获取参数需要使用#{}或${}吗?
  - 



#### 04动态语句标签 - where【掌握】

##### 目标

- ==只根据 **性别** 查找用户==

##### 步骤

1. 测试
2. **使用where标签解决问题**

1. **测试**

##### 操作

###### 配置where

```xml

```

###### 测试

```java

```

##### 小结

- where标签的作用是什么?
  - 
- 使用where要注意什么?
  - 



#### 05动态语句标签 - set【掌握】

##### 目标

- ==更新用户地址和性别==
- ==只更新用户性别==

##### 步骤

1. UserMapper.java: 增加 update 操作方法
2. userMapper.xml: 增加 update 操作配置
3. 测试
4. **使用set解决问题**
5. **测试**

##### 操作

###### 配置set

```xml

```

###### 测试

```xml

```

##### 小结

- set标签的作用是什么?
  - 
- 使用set要注意什么?
  - 



#### 06动态语句标签 - sql【了解】

##### 目标

- ==查询时返回部分字段==

##### 步骤

1. 指定字段
2. 思考字段部分是否重复
3. **使用sql, includes标签**
4. **测试**

##### 操作

###### 配置sql

```xml

```

###### 测试

```java

```

##### 小结

- sql和include标签的作用是什么?
  - 
- 为什么要使用sql标签定义sql片段?
  - 



#### 07动态语句标签 - foreach【掌握】

##### 目标

- ==查询id集合中的用户==

##### 步骤

1. UserMapper.java: 增加 **findByIds** 操作方法
2. userMapper.xml: 增加 **findByIds** 操作配置
3. **测试**

##### 操作

###### 增加 findByIds 操作方法

```java

```

###### 增加 findByIds 操作配置

```

```

###### 测试

```java

```

##### 小结

- foreach标签的作用是什么?
  - 
- 使用foreach取值时数组和集合有什么不同?
  - 



#### 08关联查询 - O2O【掌握】

##### 目标

- ==查找所有订单并显示关联用户信息==

##### 步骤

1. 创建项目, 引入依赖
2. 分析执行语句
3. 在订单类中**引用用户类**
4. **配置关联映射**
5. **测试**

##### 操作

###### 关系映射

```xml

```

###### 测试

```java

```

##### 小结

- 1对1关联可以使用resultType实现吗?
  - 
- ResultMap如何配置关联对象?
  - 





#### 09关联查询 - O2M【掌握】

##### 目标

- ==查找所有用户并显示关联订单信息==

##### 步骤

1. 分析执行语句
2. 在用户类中**引用订单集合**
3. **配置映射**
4. **测试**

##### 操作

###### 用户类

```java

```

###### 操作方法

```java

```

###### 关系映射

```xml

```

###### 测试

```

```

##### 小结

- 1对多关联可以使用resultType实现吗?
  - 
- ResultMap如何配置关联集合?
  - 



#### 10关联查询 - M2M【掌握】

##### 目标

- 掌握多对多关联映射配置

##### 步骤

- ==查询所有用户并显示关联角色信息==

1. 执行课前资料语句
2. 分析查询语句
3. **创建角色类**
4. 引入角色集合
5. UserMapper.java: 增加 **findO2O** 操作方法
6. userMapper.xml: 增加 **findO2O** 操作配置
7. **测试**

##### 操作

###### 角色类

```java

```

###### 增加 findO2O 操作方法

```java

```

###### 增加 findO2O 操作配置

```xml

```

###### 测试

```java

```

##### 小结

- 多对多与1对多有什么不同?





#### 11关联查询 - 注解方式【理解】

##### 目标

- 如何使用注解方式代替1对1关系的映射

##### 步骤

- ==查询所有订单==

1. OrderMapper.java: 增加 **findO2O** 操作方法
2. UrderMapper.java: 增加 **findO2M** 操作方法
3. 使用注解代替orderMapper.xml中的ResultMap标签
4. 使用注解代替userMapper.xml中的ResultMap标签
5. 测试

##### 操作

###### 配置方式

```xml

```

###### 测试

```java

```

###### 注解方式

```java

```



##### 小结

- @Results注解相当于映射文件中那个标签?

  

- 使用哪个注解或属性声明了1对1或1对多的关系?



#### 12关联查询 - 延迟加载【理解】

##### 目标

- 按需加载关联对象

##### 步骤

- ==查询所有订单**按需加载用户**==
- ==查询所有用户**按需加载订单**==

1. OrderMapper.java: 增加 **findO2O** 操作方法
2. UserMapper.java: 增加 **findO2M** 操作方法
3. 改造orderMapper.xml中的ResultMap标签
4. 改造userMapper.xml中的ResultMap标签
5. 测试

##### 操作

###### 配置方式

```xml

```

###### 测试

```java

```



###### 注解方式

```java

```



##### 小结

- @Many的作用是什么?

  

  

#### 13总结【了解】

##### 连接池

- 数据源可以配置哪些类型?
  - 

##### 动态SQL

- 什么是动态SQL?
  - 
- 哪些标签可以根据参数的不同拼接不同的执行语句?
  - 

##### 关联查询

- 1对1查询有哪几种实现方式?
  - 
- 1对多查询如何配置映射?
  - 
- 多对多与多对一是什么关系?
  - 



##### 注解开发

- 注解方式与映射方式的区别是什么?
  - 



##### 延迟加载

- 分批查询, 在需要的时候再查询关联对象