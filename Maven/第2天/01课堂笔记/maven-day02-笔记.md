...
- 概念: [Apache Maven](http://maven.apache.org/) 是一种 **项目管理工具**
- 模型: 通过将项目信息 封装成 **pom.xml** (项目对象模型)
- 功能: 最强大的功能就是能够 **自动下载jar包** 
...
- 依赖传递是 **默认行为**, 那么如果项目中 **用不上** 的依赖传递下来不是浪费内存吗?
  - 依赖传递是可以阻止的
- 项目中会 **依赖很多** jar包, 它们的所有依赖都会传递下来吗? 会不会 **有重复** 的?
  - 会的, 有重复的这种情况称之为版本冲突
